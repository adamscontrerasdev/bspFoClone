...
import {
  ListaParaNewListOfBeats,
  useGetBeatsQuery,
  useGetInitialBeatsQuery,
} from "../redux/service/music";
const apiUrl =
...
const Beats: React.FC = () => {
  ...
  const paginatedData: any = Array.isArray(loadedData?.documents)
    ? loadedData?.documents.slice(startIndex, endIndex) || []
    : [];
  const totalPages = Math.ceil(
    (loadedData?.documents?.length || 0) / itemsPerPage
  );
  ...
  useEffect(() => {
    ...
    if (filtroGenero && filtroGenero !== "todos") {
      filteredData = data?.documents.filter(
        (beat) => beat.fields.genero.stringValue === filtroGenero.toLowerCase()
      );
    }

    if (
      (filtroGenero && filtroGenero === "vacio") ||
      filtroGenero === "todos"
    ) {
    ...
    if (
      filtroEscala != "todos" ||
      filtroEscala != null ||
      filtroEscala != undefined
    ) {
      // Aplicar filtro de escala
      if (filtroEscala && filtroEscala !== "todos") {
        if (filtroEscala === "menor") {
          filteredData = filteredData.filter((beat) =>
            beat.fields.key.stringValue.includes("m")
          );
        }
        if (filtroEscala === "mayor") {
          filteredData = filteredData.filter(
            (beat) => !beat.fields.key.stringValue.includes("m")
          );
        }
      }
    }
      ...
      <div className={style.bgDynamic}>
        <div className={style.beatContainer}>
          <div className={`${style.filters}`} ref={cajaDeFiltros}>
            ...
            {filtrosAplicados && (
              <div className={style.filtrosAplicados}>
                {/* <h4>{filtroGenero && "Genero: " + filtroGenero}{filtroEscala && ", Escala: " + filtroEscala}</h4> */}
                {filtroGenero !== "vacio" && (
                  <div
                    className={style.burbujaDefiltro1}
                    onClick={() => limpiar("Genero")}
                  >
                    {filtroGenero}
                  </div>
                )}
                {filtroEscala !== "vacio" && (
                  <div
                    className={style.burbujaDefiltro2}
                    onClick={() => limpiar("Escala")}
                  >
                    {filtroEscala}
                  </div>
                )}
          ...
          <div className={style.container_list}>
            ...
            {loadedData?.documents?.length > 0 &&
              paginatedData.map((beat) => (
                <div
                  key={beat.name}
                  className={styless.cardOfBeat}
                  onClick={() => reproducirBeat(beat.name)}
                >
                  <img
                    src={beat.fields.pic.stringValue}
                    alt={beat.fields.pic.stringValue}
                  />
                  <span className={styless.lineOfSep1}></span>
                  <h2 className={styless.name}>
                    {beat.fields.nombre.stringValue}
                  </h2>
                  {/* <span className={styless.lineOfSep2}></span> */}
                  <h2 className={styless.genero}>
                    {beat.fields.genero.stringValue}
                  </h2>
                  <h2 className={styless.genero}>
                    {beat.fields.key.stringValue}
                  </h2>
                  <span className={styless.lineOfSep3}></span>
                  <div className={styless.bpm}>
                    <h2>{beat.fields.bpm.stringValue}</h2>
                  </div>
                </div>
              ))}
          </div>
          <div className={style.pagination}>{paginationButtons}</div>
        </div>
